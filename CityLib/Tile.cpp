...
/**
 * Accepts a tile to visit its neighboring tiles
 * @param visitor
 */
void Tile::NeighborAccept(TileVisitor* visitor)
{
    auto ul = this->GetAdjacent(-1, -1);
    auto ur = this->GetAdjacent(1, -1);
    auto ll = this->GetAdjacent(-1, 1);
    auto lr = this->GetAdjacent(1, 1);
