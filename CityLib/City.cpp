...
void City::Save(const wxString &filename)
{
    ...
    if(mViewPopulation)
    {
        root->AddAttribute(L"view-population", "yes");
    }
...
void City::Load(const wxString &filename)
{
    ...
    auto vp = root->GetAttribute(L"view-population", "no");
    mViewPopulation = vp == "yes";
...
void City::XmlTile(wxXmlNode* node)
{
    ...
    {
        tile = make_shared<TileHive>(this);
    }
    else if(type == L"water")
    {
        tile = make_shared<TileWater>(this);
    }
